...
def get_moon_from_body(time):
    """使用get_body获取月亮位置的备用方案"""
    try:
        return get_body('moon', time)
    except Exception as e:
        raise Exception(f"无法获取月亮位置: {e}")
...
def get_observation_time(time_str: str = None, timezone_str: str = "UTC") -> Time:
    ...
    if timezone_str != "UTC" and HAS_PYTZ:
        try:
...
async def get_bright_stars(latitude: float, longitude: float, time_str: str = None,
    ...
    try:
        ...
        if visible_stars:
            result += f"当前可见的亮星 ({len(visible_stars)}颗):\n" + "\n".join(visible_stars)
        else:
